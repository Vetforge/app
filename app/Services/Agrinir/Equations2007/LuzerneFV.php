...
{
    /**
     * Modifier les valeurs alimentaires d'un fourrage vert de luzerne en fonction d'une analyse Agrinir
     *
     *
    ...
    public static function modifierValeursAlimentaires(
        ...
        // PANDI=7,9+0,08MAT-0,00033MAT*MAT-1,9-2
        $PANDI = (7.9 + (0.08 * $MATgKgMS) - (0.00033 * $MATgKgMS * $MATgKgMS) - 1.9 - 2);
        // MO=MS-MM (g/Kg de MB)
        $MOgKgMB = ($MS - $MM);
        ...
        // Ebo=4531+1,735*MATo+82
        $EBo2 = ($EBo1);
        // DT_N=51,2+0,14*MAT-0,00017MAT*MAT+6,8
        $DT_N = (51.2 + (0.14 * $MATgKgMS) - (0.00017 * $MATgKgMS * $MATgKgMS) + 6.8);
        ...
        // PDIN=PDIA+PDIMN
        $PDIN = ($PDIA + $PDIMN);
        // QIM=-16+0,806*dMO+0,115MAT-0,686MS+4,2
        $QIM = (-16 + (0.806 * $dMO) + (0.115 * ($MATgKgMS)) - (0.686 * $MS100) + 4.2);
        // QIL=66,3+0,655*dMO+0,098MAT-0,626MS+1
        $QIL = (66.3 + (0.655 * $dMO) + (0.098 * ($MATgKgMS)) - (0.626 * $MS100) + 1);
        // QIB=6,44+0,782*dMO+0,112MAT-0,679MS+4,1
        $QIB = (6.44 + (0.782 * $dMO) + (0.112 * ($MATgKgMS)) - (0.679 * $MS100) + 4.1);
        // UEM=75/QIM
        $UEM = (75 / $QIM);
        // UEL=140/QIL
        $UEL = (140 / $QIL);
