...
{
    /**
     * Modifier les valeurs alimentaires d'un enrubanné d'herbe Prairie Permanente première coupe en fonction d'une analyse Agrinir
     *
     *
    ...
    public static function modifierValeursAlimentaires(
        ...
        // CB (g/kgMS)=1.19*ADF (g/kgMS)-88
        $CBkgMS = ((1.19 * $adfgKgMS) - 88);
        ...
        // MS<50 dMO%=116,5-0,148*ADF(g/kgMS)
        if ($MS100 < 50) {
            $dMO = (116.5 - (0.148 * $adfgKgMS));
        }
        // MS>50 dMO%=58,5-0,026*ADF+0,104*MAT
        if ($MS100 >= 50) {
            $dMO = (58.5 - (0.026 * $adfgKgMS) + (0.104 * $MATgKgMS));
        }
        ...
        // MS<50 EBo=1,03*(4531+1,735*MATo+82)
        if ($MS100 < 50) {
            $EBo1 = (1.03 * (4531 + (1.735 * $MATo) + 82));
        }
        // MS>50 EBo=4531+1,735*MATo+82
        if ($MS100 >= 50) {
            $EBo1 = (4531 + (1.735 * $MATo) + 82);
        }
        // EBo=4531+1,735*MATo+82
        $EBo2 = ($EBo1);
        // MS<50 DT%=73,7+0,088MAT-0,00011MAT*MAT-0,25MS+2,5
        if ($MS100 < 50) {
            $DT_N = (73.7 + (0.088 * $MATgKgMS) - (0.00011 * $MATgKgMS * $MATgKgMS) - (0.25 * $MS100) + 2.5);
        }
        // MS>50 DT%=50,8+0,12*MAT-0,00018MAT*MAT+1,9
        if ($MS100 >= 50) {
            $DT_N = (50.8 + (0.12 * $MATgKgMS) - (0.00018 * $MATgKgMS * $MATgKgMS) + 1.9);
        }
        ...
        if ($MS100 >= 50) {
            // MS=25 MOF=MOD-MG-(MAT*(1-DT/100))-(140/1000*MS)
            $MOF25 = ($MOD - $MG - $MATgKgMB * (1 - $DT_N / 100) - 140 / 1000 * $MS);
            // MS=30 MOF=MOD-MG-(MAT*(1-DT/100))-(140/1000*MS)
            $MOF30 = ($MOD - $MG - $MATgKgMB * (1 - $DT_N / 100) - 140 / 1000 * $MS);
            // MS=35 MOF=MOD-MG-(MAT*(1-DT/100))-(140/1000*MS)
            $MOF35 = ($MOD - $MG - $MATgKgMB * (1 - $DT_N / 100) - 140 / 1000 * $MS);
            // MS=40 MOF=MOD-MG-(MAT*(1-DT/100))-(140/1000*MS)
            $MOF40 = ($MOD - $MG - $MATgKgMB * (1 - $DT_N / 100) - 140 / 1000 * $MS);
        }
        ...
        $PDIN = ($PDIA + $PDIMN);
        if ($MS100 < 50) {
            // QIM=20,1+0,306*dMO+0,078MAT-3,7
            $QIM = (20.1 + (0.306 * $dMO) + (0.078 * ($MATgKgMS)) - 3.7);
            // QIL=99,3+0,167*dMO+0,128MAT+1,6
            $QIL = (99.3 + (0.167 * $dMO) + (0.128 * ($MATgKgMS)) + 1.6);
            // QIB=47+0,228*dMO+0,148MAT+1,9
            $QIB = (47 + (0.228 * $dMO) + (0.148 * ($MATgKgMS)) + 1.9);
        }
        if ($MS100 >= 50) {
            // QIM=11,8+0,432*dMO+0,100MAT+2,9
            $QIM = (11.8 + (0.432 * $dMO) + (0.100 * ($MATgKgMS)) + 2.9);
            // QIL=82,4+0,491*dMO+0,114MAT+5,5
            $QIL = (82.4 + (0.491 * $dMO) + (0.114 * ($MATgKgMS)) + 5.5);
            // QIB=30,3+0,559*dMO+0,132MAT+5,2
            $QIB = (30.3 + (0.559 * $dMO) + (0.132 * ($MATgKgMS)) + 5.2);
        }
        // UEM=75/QIM
        $UEM = (75 / $QIM);
        // UEL=140/QIL
