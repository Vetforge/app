...
{
    /**
     * Modifier les valeurs alimentaires d'une ensilage de luzerne sans conservateur en fonction d'une analyse Agrinir
     *
     *
    ...
    public static function modifierValeursAlimentaires(
        ...
        // CB (g/kgMB)=CB(g/kgMS)*MS(g/kgMB)/1000
        ...
        // dE%=1,0263dMO-5,723
        $dE = ((1.0263 * $dMO) - 5.723);
        // Ebo1 (kcal/kgMO)=1,03*(4531+1,735*MATo+82)
        $EBo1 = (1.03 * (4531 + (1.735 * $MATo) + 82));
        // Ebo2 (kcal/kgMO)=1,03*(4531+1,735*MATo+82)
        $EBo2 = ($EBo1);
        // DT_N=73,7+0,088MAT-0,00011MAT*MAT-0,25MS(%)+4,2
        $DT_N = (73.7 + (0.088 * $MATgKgMS) - (0.00011 * $MATgKgMS * $MATgKgMS) - (0.25 * $MS100) + 4.2);
        // dr%=100*(1,11*(1-DT%/100)*MAT-PANDI)/(1,11*(1-DT%/100)*MAT)
        $dr = (100 * ((1.11 * (1 - ($DT_N / 100)) * $MATgKgMS) - $PANDI) / (1.11 * (1 - ($DT_N / 100)) * $MATgKgMS));
        // MS<30 EB1 (kcal/kgMB)=EBo1*MO/1000
        $EB1 = ($EBo1 * $MOgKgMB / 1000);
        // MS>30 EB2 (kcal/kgMB)=EBo2*MO/1000
        $EB2 = ($EBo2 * $MOgKgMB / 1000);
        // EM (kcal/kgMB)=EB*dE/100*(EM/ED)
        $EM = ($EB2 * $dE * $EMED / 100);
        ...
        // ENL (kcal/kgMB)=EM*kl
        $ENL = ($EM * $kl);
        // ENEV (kcal/kgMB)=EM*kmf
        $ENEV = ($EM * $kmf);
        // UFL (/kgMB)=ENL/1700
        $UFL = ($ENL / 1700);
        // UFV (/kgMB)=ENEV/1820
        $UFV = ($ENEV / 1820);
        // PDIA (g/kgMB)=MAT*(1,11*(1-DT/100))*dr/100
        $PDIA = ($MATgKgMB * (1.11 * (1 - ($DT_N / 100))) * $dr / 100);
        // PDIMN=MAT*(1-1,11*(1-DT/100))*0,9*0,8*0,8
        $PDIMN = ($MATgKgMB * (1 - (1.11 * (1 - $DT_N / 100))) * 0.9 * 0.8 * 0.8);
        // MOD=MO*dMO/100
        $MOD = ($MOgKgMB * $dMO / 100);
        // MS=25 MOF=MOD-MG-(MAT*(1-DT/100))-(155/1000*MS)
        $MOF25 = ($MOD - $MG - $MATgKgMB * (1 - $DT_N / 100) - 155 / 1000 * $MS);
        // MS=30 MOF=MOD-MG-(MAT*(1-DT/100))-(155/1000*MS)
        $MOF30 = ($MOD - $MG - $MATgKgMB * (1 - $DT_N / 100) - 155 / 1000 * $MS);
        // MS=35 MOF=MOD-MG-(MAT*(1-DT/100))-(155/1000*MS)
        $MOF35 = ($MOD - $MG - $MATgKgMB * (1 - $DT_N / 100) - 155 / 1000 * $MS);
        // MS=40 MOF=MOD-MG-(MAT*(1-DT/100))-(155/1000*MS)
        $MOF40 = ($MOD - $MG - $MATgKgMB * (1 - $DT_N / 100) - 155 / 1000 * $MS);
        // PDIME=MOF*0,145*0,8*0,8
        $PDIME = ($MOF35 * 0.145 * 0.8 * 0.8);
        // PDIE (g/kgMB)=PDIA+PDIME
        $PDIE = ($PDIA + $PDIME);
        // PDIN (g/kgMB)=PDIA+PDIMN
        $PDIN = ($PDIA + $PDIMN);
