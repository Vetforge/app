...
{
    /**
     * Modifier les valeurs alimentaires d'un ensilage de maïs en fonction d'une analyse Agrinir
     *
     *
    ...
    public static function modifierValeursAlimentaires(
        ...
        // dE%=1,001dMO-2,86
        $dE = ((1.001 * $dMO) - 2.86);
        // MS<3O EBo1=1,02*(4487+2,019*MATo)
        $EBo1 = (1.02 * (4487 + (2.019 * $MATo)));
        // MS>30 EBo2=(4487+2,019*MATo)+25
        $EBo2 = ((4487 + (2.019 * $MATo)) + 25);
        // DT_N=72
        $DT_N = 72;
        // dr%=70
        $dr = 70;
        ...
        // MOD=MO*dMO/100
        $MOD = ($MOgKgMB * $dMO / 100);
        // MS=25 MOF=MOD-MG-(MAT*1.11*(1-DT/100))-(125/1000*MS)
        $MOF25 = ($MOD - $MG - $MATgKgMB * 1.11 * (1 - $DT_N / 100) - 125 / 1000 * $MS);
        // MS=30 MOF=MOD-MG-(MAT*1.11*(1-DT/100))-(100/1000*MS)
        $MOF30 = ($MOD - $MG - $MATgKgMB * 1.11 * (1 - $DT_N / 100) - 100 / 1000 * $MS);
        // MS=35 MOF=MOD-MG-(MAT*1.11*(1-DT/100))-(80/1000*MS)
        $MOF35 = ($MOD - $MG - $MATgKgMB * 1.11 * (1 - $DT_N / 100) - 80 / 1000 * $MS);
        // MS=40 MOF=MOD-MG-(MAT*1.11*(1-DT/100))-(60/1000*MS)
        $MOF40 = ($MOD - $MG - $MATgKgMB * 1.11 * (1 - $DT_N / 100) - 60 / 1000 * $MS);
        $PDIME = 0.0;
