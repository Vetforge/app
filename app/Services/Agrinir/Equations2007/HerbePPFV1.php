...
{
    /**
     * Modifier les valeurs alimentaires d'un fourrage vert d'herbe Prairie Permanente première coupe en fonction d'une analyse Agrinir
     *
     *
    ...
    public static function modifierValeursAlimentaires(
        ...
        // PANDI=7,9+0,08MAT-0,00033MAT*MAT-1,9-2,3-2
        $PANDI = (7.9 + (0.08 * $MATgKgMS) - (0.00033 * $MATgKgMS * $MATgKgMS) - 1.9 - 2.3 - 2);
        // MO=MS-MM (g/Kg de MB)
        $MOgKgMB = ($MS - $MM);
        ...
        // dMO%=99-0,115*ADF+0,043*MAT
        $dMO = (99 - (0.115 * $adfgKgMS) + (0.043 * $MATgKgMS));
        // dE%=0,957*dMO-0,068
        $dE = ((0.957 * $dMO) - 0.068);
        ...
        // EBo=4531+1,735*MATo+82
        $EBo2 = ($EBo1);
        // DT_N=51,2+0,14*MAT-0,00017MAT*MAT+4,4
        $DT_N = (51.2 + (0.14 * $MATgKgMS) - (0.00017 * $MATgKgMS * $MATgKgMS) + 4.4);
        ...
        // PDIN=PDIA+PDIMN
        $PDIN = ($PDIA + $PDIMN);
        // QIM=-16+0,806*dMO+0,115MAT+0,686MS
        $QIM = (-16 + (0.806 * $dMO) + (0.115 * ($MATgKgMS)) + (0.686 * $MS100));
        // QIL=66,3+0,655*dMO+0,098MAT+0,626MS
        $QIL = (66.3 + (0.655 * $dMO) + (0.098 * ($MATgKgMS)) + (0.626 * $MS100));
        // QIB=6,44+0,782*dMO+0,112MAT+0,679MS
        $QIB = (6.44 + (0.782 * $dMO) + (0.112 * ($MATgKgMS)) + (0.679 * $MS100));
        // UEM=75/QIM
        $UEM = (75 / $QIM);
        // UEL=140/QIL
        $UEL = (140 / $QIL);
