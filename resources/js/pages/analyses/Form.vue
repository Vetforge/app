...
const form = reactive<FormState>({
    ...
    analyzed_at:
        props.analysis?.analyzed_at?.slice(0, 10) ??
...
const coproscopySpeciesOptions = computed(() => {
    const configured = Array.isArray(props.settings.species_options)
    ...
    return ['Bovin', 'Ovin', 'Caprin', 'Equin', 'Chien', 'Chat'].map(
        (species) => ({ value: species, label: species }),
    );
});
const parasiteRows = computed(() =>
    enabledItems('parasites').filter((parasite) =>
        coproscopyParasiteMatches(parasite),
    ),
);
const parasiteRowKeys = computed(() =>
    parasiteRows.value.map((parasite) => parasite.key).join('|'),
);
const pathogenRows = computed(() => enabledItems('pathogens'));
const perfusionRows = computed(() => enabledItems('perfusions'));
const antibioticRows = computed(() => enabledItems('antibiotics'));
const germFamilies = computed<string[]>(
    () => props.settings.germ_families ?? [],
);
const scale = computed<Array<{ value: string; label: string }>>(
    () => props.settings.scale ?? [],
);
const bloodGasInputFields = [
    { key: 'weight', label: 'Poids', unit: 'kg', step: '0.1' },
    { key: 'enophtalmie', label: 'Enophtalmie', unit: 'mm', step: '0.1' },
    {
        key: 'dehydration',
...
const bloodGasLabFields = bloodGasInputFields.slice(3);
...
const bloodGasSpeciesOptions = computed(() => {
    const options = Array.isArray(props.settings.species_options)
        ? props.settings.species_options
        : [];
        ...
        return options
            .map((option: Record<string, any>) => ({
                ...
                calculationProfile: String(
                    option.calculation_profile ??
                        defaultBloodGasProfile(String(option.value ?? '')),
                ),
            }))
            .filter((option) => option.value !== '');
    }
...
const selectedBloodGasSpecies = computed(() => {
    const species = String(form.payload.species ?? '');

    return (
        bloodGasSpeciesOptions.value.find(
            (option) => option.value === species,
        ) ??
        bloodGasSpeciesOptions.value[0] ?? {
            value: species || 'Bovin',
            label: species || 'Bovin',
...
const selectedBloodGasNorms = computed(() => {
    const norms = isRecord(props.settings.norms) ? props.settings.norms : {};
    ...
    const candidates = [
        selectedBloodGasSpecies.value.normKey,
        selectedBloodGasSpecies.value.value,
        legacyKeys[selectedBloodGasSpecies.value.value],
    ].filter(Boolean);

    for (const candidate of candidates) {
        if (isRecord(norms[candidate])) {
            return normalizeBloodGasNorms(norms[candidate]);
        }
    }

    return {};
});
const bloodGasNormRows = computed(() =>
    bloodGasNormFields.map((field) => {
        const range = selectedBloodGasNorms.value[field.key];
...
const bloodGasCalculation = computed(() => {
    const profile = selectedBloodGasSpecies.value.calculationProfile;
    ...
    const dehydration =
        profile === 'ruminant'
            ? (explicitDehydration ??
              (enophtalmie === null ? null : round1(1.7 * enophtalmie + 0.38)))
            : explicitDehydration;
    const baseHco3 = profile === 'ruminant' ? 24.8 : 25.6;
    const basePh = profile === 'ruminant' ? 7.4 : 7.37;
    const volumeDeficit =
        weight === null || dehydration === null
            ? null
            : round1((weight * dehydration) / 100);
    const bicarbonateDeficit =
        weight === null || ph === null || hco3 === null
            ? null
            : Math.round(
                  -(
                      ((hco3 - baseHco3 + 16.2 * (ph - basePh)) *
                          (weight * 0.6) *
                          84) /
                      1000
                  ),
              );
    const glucoseDeficit =
        glycemia === null
            ? null
            : profile === 'ruminant'
              ? glycemia < 54
    ...
    const apports = perfusionRows.value.reduce(
        (totals, perfusion) => {
            const quantity =
                toNumber(form.payload.perfusions?.[perfusion.key]) ?? 0;

            totals.bicarbonateG +=
                quantity * (toNumber(perfusion.bicarbonate) ?? 0);
        ...
        restes: {
            bicarbonateG:
                bicarbonateDeficit === null
                    ? null
                    : Math.round(bicarbonateDeficit - apports.bicarbonateG),
            glucoseG:
                glucoseDeficit === null
                    ? null
                    : Math.round(glucoseDeficit - apports.glucoseG),
            volumeL:
                volumeDeficit === null
                    ? null
                    : round1(volumeDeficit - apports.volumeL),
        },
    };
});
...
function coproscopyParasiteMatches(parasite: Record<string, any>): boolean {
    ...
    return (
        requiredOption === '' || form.payload.options?.[requiredOption] === true
    );
}
...
function normalizeBloodGasNorms(
    ...
    return Object.fromEntries(
        bloodGasNormFields
            ...
            .filter(
                (entry): entry is [string, [number, number]] => entry !== null,
            ),
    );
}
...
function setSampleCount(count: number): void {
    ...
    const current = Array.isArray(form.payload.samples)
        ? form.payload.samples
        : [];
        ...
        if (props.module.slug === 'coproscopie-parasitaire') {
            return {
                ...
                results: Object.fromEntries(
                    parasiteRows.value.map((parasite) => [
                        parasite.key,
                        existing.results?.[parasite.key] ?? '0',
                    ]),
                ),
            };
        }
...
function setGermCount(count: number): void {
    ...
    form.payload.germs = Array.from({ length: entryCount }, (_, index) => {
        ...
        return {
            family: existing.family ?? germFamilies.value[0] ?? 'Autre',
...
function submit(): void {
    ...
    if (props.analysis) {
        router.put(
            analysisUpdate({ analysis: props.analysis.id }).url,
            data,
            options,
        );
        return;
    ...
    router.post(
        analysisStore({ module: props.module.slug }).url,
        data,
        options,
    );
}
...
watch(
    ...
    (count) => {
        if (
            props.module.slug === 'coproscopie-parasitaire' ||
            props.module.slug === 'comptage-cellulaire'
        ) {
            setSampleCount(Number(count || 1));
        }
    },
);

watch(
    coproscopySpeciesOptions,
    (options) => {
        if (
            props.module.slug === 'coproscopie-parasitaire' &&
            options.length > 0 &&
            !options.some((option) => option.value === form.payload.species)
        ) {
...
watch(
    ...
    (isComptage) => {
        if (props.module.slug !== 'coproscopie-parasitaire') return;
...
watch(
    ...
    (count) => {
        if (props.module.slug === 'diagnostic-bacteriologique') {
            setGermCount(Number(count ?? 0));
        }
    },
);

watch(
    bloodGasSpeciesOptions,
    (options) => {
        if (
            props.module.slug === 'gaz-du-sang' &&
            options.length > 0 &&
            !options.some((option) => option.value === form.payload.species)
        ) {
            form.payload.species = options[0].value;
...
<template>
    <Head
        :title="
            isEdit
                ? `Modifier ${module.short_label}`
                : `Nouvelle ${module.short_label}`
        "
    />

    <AppLayout :breadcrumbs="breadcrumbs">
        <form
            class="analysis-form mx-auto flex max-w-7xl flex-col gap-3 p-3 sm:p-4"
        ...
        >
            ...
            >
                <div>
                    <h1 class="text-xl font-bold text-foreground">
                        {{ isEdit ? 'Modifier analyse' : 'Nouvelle analyse' }}
                    </h1>
            ...
            <section class="analysis-section">
                ...
                <div class="grid gap-2 md:grid-cols-2 xl:grid-cols-12">
                    <div class="grid gap-1 xl:col-span-4">
                        <label class="text-sm font-medium" for="breeder_id"
                            >Eleveur *</label
                        >
                        <BreederSelectWithCreate
                            v-model="form.breeder_id"
                            input-id="breeder_id"
                            :breeders="breeders"
                    ...
                    <div class="grid gap-1 xl:col-span-2">
                        <label class="text-sm font-medium" for="animal_nom"
                            >Animal</label
                        >
                        <input
                            id="animal_nom"
                            v-model="form.animal_nom"
                    ...
                    <div class="grid gap-1 xl:col-span-2">
                        <label class="text-sm font-medium" for="sampled_at"
                            >Prelevement</label
                        >
                        <input
                            id="sampled_at"
                            v-model="form.sampled_at"
                            type="date"
                            class="h-10 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm"
                        />
                    ...
                    <div class="grid gap-1 xl:col-span-2">
                        <label class="text-sm font-medium" for="analyzed_at"
                            >Analyse</label
                        >
                        <input
                            id="analyzed_at"
                            v-model="form.analyzed_at"
                            type="date"
                            class="h-10 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm"
                        />
                    ...
                    <div class="grid gap-1 xl:col-span-2">
                        <label class="text-sm font-medium" for="intervenant"
                            >Intervenant</label
                        >
                        <input
                            id="intervenant"
            ...
            >
                ...
                <div class="grid gap-2 lg:grid-cols-[1fr_1fr_10rem_18rem]">
                    <div class="grid gap-1">
                        <label class="text-sm font-medium" for="species"
                            >Espece</label
                        >
                        <select
                            id="species"
                            v-model="form.payload.species"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                        >
                            <option
                                v-for="species in coproscopySpeciesOptions"
                                :key="species.value"
                                :value="species.value"
                            >
                                {{ species.label }}
                    ...
                    <div class="grid gap-1">
                        <label class="text-sm font-medium" for="sample_nature"
                            >Nature</label
                        >
                        <input
                            id="sample_nature"
                            v-model="form.payload.sample_nature"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                        />
                    </div>
                    <div class="grid gap-1">
                        <label class="text-sm font-medium" for="sample_count"
                            >Echantillons</label
                        >
                        <input
                            id="sample_count"
                            v-model.number="form.payload.sample_count"
                            type="number"
                            min="1"
                            max="5"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                    ...
                    <div
                        class="grid gap-1 rounded-md border border-border bg-muted/30 p-2"
                    >
                        ...
                        <div class="grid gap-1 sm:grid-cols-3 lg:grid-cols-1">
                            <label
                                ...
                                ><input
                                    v-model="form.payload.options.dictyocaules"
                                    type="checkbox"
                                />
                                Dictyocaules</label
                            >
                            <label
                                ...
                                ><input
                                    v-model="
                                        form.payload.options.cryptosporidies
                                    "
                                    type="checkbox"
                                />
                                Cryptosporidies</label
                            >
                            <label
                                class="flex items-center gap-1.5 text-xs text-muted-foreground"
                                ><input
                                    v-model="form.payload.options.comptage"
                                    type="checkbox"
                                />
                                Comptage</label
                            >
                ...
                <div class="analysis-table-wrap">
                    <table class="w-full min-w-[760px] text-sm">
                        <thead>
                            <tr class="border-b border-border">
                                <th class="px-2 py-1 text-left font-medium text-xs">
                                    Echantillon
                                    <span
                                        v-if="form.payload.options.comptage"
                                ...
                                <th
                                    v-for="parasite in parasiteRows"
                                    :key="parasite.key"
                                    class="px-2 py-1 text-left font-medium text-xs"
                                >
                                    {{ parasite.label }}
                                </th>
                            </tr>
                        ...
                        <tbody>
                            <tr
                                v-for="(sample, sampleIndex) in form.payload
                                    .samples"
                                :key="sampleIndex"
                            ...
                            >
                                <td class="px-2 py-1">
                                    <input
                                        ...
                                        :placeholder="`Echantillon ${Number(sampleIndex) + 1}`"
                                    />
                                </td>
                                <td
                                    v-for="parasite in parasiteRows"
                                ...
                                >
                                    <input
                                        v-if="form.payload.options.comptage"
                                        v-model.number="sample.results[parasite.key]"
                                        type="number"
                                        min="0"
                                        step="1"
                                        class="w-full rounded border border-border bg-background px-2 py-1"
                                    ...
                                    <select
                                        v-else
                                        v-model="sample.results[parasite.key]"
                                        class="w-full rounded border border-border bg-background px-2 py-1"
                                    >
                                        <option
                                            v-for="choice in scale"
                                            :key="choice.value"
                                            :value="choice.value"
                                        >
                                            {{ choice.label }}
            ...
            >
                <div class="analysis-section-heading">
                    <div>
                        ...
                        <p>
                            Test, prelevement et lecture des agents recherches.
                        </p>
                    </div>
                </div>
                ...
                <div class="grid gap-2 lg:grid-cols-[1fr_1fr_1fr_auto]">
                    <div class="grid gap-1">
                        <label class="text-sm font-medium" for="test_name"
                            >Test utilise</label
                        >
                        <select
                            id="test_name"
                            v-model="form.payload.test_name"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                        >
                            <option
                                v-for="test in settings.tests"
                                :key="test"
                                :value="test"
                            >
                                {{ test }}
                    ...
                    <div class="grid gap-1">
                        <label
                            ...
                            for="sample_nature_diarrhea"
                            >Nature</label
                        >
                        <input
                            id="sample_nature_diarrhea"
                            v-model="form.payload.sample_nature"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                        />
                    </div>
                    <div class="grid gap-1">
                        <label class="text-sm font-medium" for="sample_name"
                            >Echantillon</label
                        >
                        <input
                            id="sample_name"
                            v-model="form.payload.sample_name"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                        />
                    </div>
                    <label
                        class="flex items-center gap-1.5 rounded-md border border-border bg-muted/30 px-2 py-1 text-xs text-muted-foreground lg:self-end"
                    >
                        <input
                            v-model="form.payload.coccidiosis_test"
                            type="checkbox"
                        />
                ...
                <div class="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
                    <div
                        v-for="pathogen in pathogenRows"
                        :key="pathogen.key"
                        class="grid gap-1.5 rounded-md border border-border bg-muted/20 p-3"
                    >
                        <label
                            class="text-sm font-medium"
                            :for="`pathogen-${pathogen.key}`"
                            >{{ pathogen.label }}</label
                        >
                        <select
                            :id="`pathogen-${pathogen.key}`"
                            v-model="form.payload.pathogens[pathogen.key]"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                        >
                            <option
                                v-for="choice in scale"
            ...
            >
                ...
                <div
                    class="grid gap-2 xl:grid-cols-[minmax(18rem,0.85fr)_minmax(0,1.6fr)]"
                >
                    <div class="analysis-subsection">
                        ...
                        <div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-1">
                            <div class="grid gap-1">
                                ...
                                >
                                    <option
                                        v-for="species in bloodGasSpeciesOptions"
                                        :key="species.value"
                                        :value="species.value"
                                    >
                                        {{ species.label }}
                            ...
                            <div class="grid gap-1">
                                ...
                                <div class="analysis-unit-field analysis-unit-field--with-unit">
                                    <input
                                        id="blood-enophtalmie"
                                        v-model="form.payload.enophtalmie"
                                        type="number"
                                        step="0.1"
                                        class="rounded border border-border bg-background px-2 py-1 text-sm"
                                    />
                            ...
                            <div class="grid gap-1">
                                ...
                                <div class="analysis-unit-field analysis-unit-field--with-unit">
                                    <input
                                        id="blood-dehydration"
                                        v-model="form.payload.dehydration"
                                        type="number"
                                        step="0.1"
                                        class="rounded border border-border bg-background px-2 py-1 text-sm"
                                        @input="form.payload.enophtalmie = null"
                                    />
                    ...
                    <div class="analysis-subsection">
                        ...
                        <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                            <div
                                v-for="field in bloodGasLabFields"
                                :key="field.key"
                                class="grid gap-1"
                            >
                                <label
                ...
                <div class="analysis-subsection">
                    ...
                    <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                        <div
                            v-for="perfusion in perfusionRows"
                            :key="perfusion.key"
                            class="grid gap-1"
                        >
                            <label
                                class="text-sm font-medium"
                                :for="`perf-${perfusion.key}`"
                                >{{ perfusion.label }}</label
                            >
                            <div
                                class="analysis-unit-field"
                                :class="{
                            ...
                            >
                                <input
                                    :id="`perf-${perfusion.key}`"
                                    v-model.number="
                                        form.payload.perfusions[perfusion.key]
                                    "
                                    type="number"
                                    min="0"
                                    step="0.1"
                ...
                <div class="grid gap-2 xl:grid-cols-[1fr_1fr]">
                    <div class="analysis-subsection">
                        <div>
                            ...
                            <p class="text-sm text-muted-foreground">
                                Profil
                                {{
                                    bloodGasCalculation.profile === 'ruminant'
                                        ? 'ruminant'
                                        : 'equin / autre'
                                }}
                            </p>
                        ...
                        <div
                            class="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-2"
                        >
                            <div class="rounded-md bg-muted/50 p-2">
                                <p
                                    class="text-[0.65rem] text-muted-foreground uppercase"
                                >
                                    Deshydratation
                                </p>
                                <p class="text-base font-semibold">
                                    {{
                                        formatResult(
                                            bloodGasCalculation.dehydration,
                                            '%',
                                        )
                                    }}
                                </p>
                            ...
                            <div class="rounded-md bg-muted/50 p-2">
                                ...
                                <p class="text-base font-semibold">
                                    {{
                                        formatResult(
                                            bloodGasCalculation.volumeDeficit,
                                            'L',
                                        )
                                    }}
                                </p>
                            ...
                            <div class="rounded-md bg-muted/50 p-2">
                                ...
                                <p class="text-base font-semibold">
                                    {{
                                        formatResult(
                                            bloodGasCalculation.bicarbonateDeficit,
                                            'g',
                                        )
                                    }}
                                </p>
                            ...
                            <div class="rounded-md bg-muted/50 p-2">
                                ...
                                <p class="text-base font-semibold">
                                    {{
                                        formatResult(
                                            bloodGasCalculation.glucoseDeficit,
                                            'g',
                                        )
                                    }}
                                </p>
                                    ...
                                    <tr class="border-b border-border/50">
                                        <td class="px-2 py-1">
                                            ...
                                            <div class="mt-0.5 flex flex-wrap gap-x-2 gap-y-0.5 text-xs text-muted-foreground sm:hidden">
                                                <span>Bicarb : {{ formatResult(bloodGasCalculation.apports.bicarbonateG, 'g') }}</span>
                                                <span>Glucose : {{ formatResult(bloodGasCalculation.apports.glucoseG, 'g') }}</span>
                                                <span>Vol : {{ formatResult(bloodGasCalculation.apports.volumeL, 'L') }}</span>
                                            </div>
                                        </td>
                                        <td class="hidden px-2 py-1 sm:table-cell">{{ formatResult(bloodGasCalculation.apports.bicarbonateG, 'g') }}</td>
                                        <td class="hidden px-2 py-1 sm:table-cell">{{ formatResult(bloodGasCalculation.apports.glucoseG, 'g') }}</td>
                                        <td class="hidden px-2 py-1 sm:table-cell">{{ formatResult(bloodGasCalculation.apports.volumeL, 'L') }}</td>
                                    </tr>
                                    <tr>
                                        <td class="px-2 py-1">
                                            <div class="font-medium">Reste a couvrir</div>
                                            <div class="mt-0.5 flex flex-wrap gap-x-2 gap-y-0.5 text-xs text-muted-foreground sm:hidden">
                                                <span>Bicarb : {{ formatResult(bloodGasCalculation.restes.bicarbonateG, 'g') }}</span>
                                                <span>Glucose : {{ formatResult(bloodGasCalculation.restes.glucoseG, 'g') }}</span>
                                                <span>Vol : {{ formatResult(bloodGasCalculation.restes.volumeL, 'L') }}</span>
                                            </div>
                                        </td>
                                        <td class="hidden px-2 py-1 sm:table-cell">{{ formatResult(bloodGasCalculation.restes.bicarbonateG, 'g') }}</td>
                                        <td class="hidden px-2 py-1 sm:table-cell">{{ formatResult(bloodGasCalculation.restes.glucoseG, 'g') }}</td>
                                        <td class="hidden px-2 py-1 sm:table-cell">{{ formatResult(bloodGasCalculation.restes.volumeL, 'L') }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div class="analysis-subsection">
                        <div>
                            <h3>Normes {{ selectedBloodGasSpecies.label }}</h3>
                            <p class="text-sm text-muted-foreground">
                                Lecture des valeurs saisies par rapport aux
                                references de l'espece.
                            </p>
                        </div>
                        <div class="analysis-table-wrap">
                            <table class="w-full text-sm">
                                ...
                                <tbody>
                                    <tr v-for="row in bloodGasNormRows" :key="row.key" class="border-b border-border/50 last:border-b-0">
                                        <td class="px-2 py-1">
                ...
                <div class="grid gap-1">
                    <label class="text-sm font-medium" for="treatment"
                        >Traitement / remarques</label
                    >
                    <textarea
                        id="treatment"
                        v-model="form.payload.treatment"
                        rows="3"
                        class="rounded border border-border bg-background px-2 py-1 text-sm"
                    ></textarea>
                </div>
            ...
            >
                ...
                <div class="grid gap-2 md:grid-cols-3">
                    <div class="grid gap-1">
                        <label class="text-sm font-medium" for="cell_species"
                            >Espece</label
                        >
                        <input
                            id="cell_species"
                    ...
                    <div class="grid gap-1">
                        <label class="text-sm font-medium" for="cell_nature"
                            >Nature</label
                        >
                        <input
                            id="cell_nature"
                    ...
                    <div class="grid gap-1">
                        <label class="text-sm font-medium" for="cell_count"
                            >Echantillons</label
                        >
                        <input
                            id="cell_count"
                ...
                <div class="grid gap-3 md:grid-cols-2">
                    <div
                        v-for="(sample, index) in form.payload.samples"
                        :key="index"
                        class="grid gap-1 rounded-md border border-border bg-muted/20 p-2"
                    >
                        <label
                            class="text-sm font-medium"
                            :for="`cell-sample-${index}`"
                            >Echantillon {{ Number(index) + 1 }}</label
                        >
                        <input
                            :id="`cell-sample-${index}`"
                            v-model="sample.name"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                            placeholder="Identification"
                        />
                        <input
                            v-model.number="sample.count"
                            type="number"
                            min="0"
                            step="1"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                            placeholder="x1000 cellules"
                        />
                    </div>
                </div>

                <div class="grid gap-1">
                    <label class="text-sm font-medium" for="cell_comments"
                        >Commentaires</label
                    >
                    <textarea
                        id="cell_comments"
                        v-model="form.payload.comments"
                        rows="3"
                        class="rounded border border-border bg-background px-2 py-1 text-sm"
                    ></textarea>
                </div>
            ...
            >
                ...
                <div class="grid gap-2 md:grid-cols-3">
                    <div class="grid gap-1">
                        <label class="text-sm font-medium" for="bact_nature"
                            >Prelevement</label
                        >
                        <input
                            id="bact_nature"
                    ...
                    <div class="grid gap-1">
                        <label
                            class="text-sm font-medium"
                            for="bact_identification"
                            >Identification</label
                        >
                        <input
                            id="bact_identification"
                            v-model="form.payload.sample_identification"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                        />
                    </div>
                    <div class="grid gap-1">
                        <label class="text-sm font-medium" for="germ_count"
                            >Nombre de germes</label
                        >
                        <input
                            id="germ_count"
                            v-model.number="form.payload.germ_count"
                            type="number"
                            min="0"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                        />
                        <span
                            v-if="form.payload.germ_count === 0"
                            class="text-xs font-medium text-green-700"
                        >→ Affichera : Sterile</span>
                        <span
                            v-else-if="form.payload.germ_count >= 3"
                            class="text-xs font-medium text-red-700"
                        >→ Affichera : Contaminé</span>
                    </div>
                </div>

                <div class="grid gap-1">
                    <label class="text-sm font-medium" for="bact_commemoratives"
                        >Commemoratifs</label
                    >
                    <textarea
                        id="bact_commemoratives"
                        v-model="form.payload.commemoratives"
                        rows="3"
                        class="rounded border border-border bg-background px-2 py-1 text-sm"
                    ></textarea>
                </div>
                ...
                <div
                    v-for="(germ, germIndex) in form.payload.germs"
                    :key="germIndex"
                ...
                >
                    <div class="grid gap-1.5 md:w-80">
                        <label
                            class="text-sm font-medium"
                            :for="`germ-family-${germIndex}`"
                            >Germe {{ Number(germIndex) + 1 }}</label
                        >
                        <select
                            :id="`germ-family-${germIndex}`"
                            v-model="germ.family"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                        >
                            <option
                                v-for="family in germFamilies"
                                :key="family"
                                :value="family"
                            >
                                {{ family }}
                    ...
                    <div class="rounded-lg border border-border">
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                <tr
                                    v-for="antibiotic in antibioticRows"
                                    :key="antibiotic.code"
                                ...
                                >
                                    ...
                                    <td class="px-2 py-1">
                                        <input
                                            ...
                                            min="0"
                                            step="1"
                                            class="w-28 rounded border border-border bg-background px-2 py-1"
                                        />
                                    </td>
                                    <td class="hidden px-2 py-1 text-muted-foreground sm:table-cell">
                                        I >= {{ antibiotic.intermediate_min }} /
                                        S >= {{ antibiotic.sensitive_min }}
                                    </td>
                                </tr>
                            </tbody>
                        </table>
            ...
            <section class="analysis-section">
                ...
                <div class="grid gap-1.5 md:grid-cols-2">
                    <div class="grid gap-1">
                        <label
                            class="text-sm font-medium"
                            for="advice_preventive"
                            >Conseils preventifs</label
                        >
                        <textarea
                            id="advice_preventive"
                            v-model="form.payload.advice_preventive"
                            rows="4"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                        ></textarea>
                    </div>
                    <div class="grid gap-1">
                        <label class="text-sm font-medium" for="advice_curative"
                            >Conseils curatifs</label
                        >
                        <textarea
                            id="advice_curative"
                            v-model="form.payload.advice_curative"
                            rows="4"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                        ></textarea>
                    </div>
                    ...
                    >
                        <label class="text-sm font-medium" for="bact_advice"
                            >Conseils bacteriologie</label
                        >
                        <textarea
                            id="bact_advice"
                            v-model="form.payload.advice"
                            rows="4"
                            class="rounded border border-border bg-background px-2 py-1 text-sm"
                        ></textarea>
                    </div>
            ...
            >
                <Link
                    :href="
                        analysis
                            ? analysisShow({ analysis: analysis.id }).url
                            : analysesIndex({ module: module.slug }).url
                    "
                ...
                <button
                    type="submit"
                    :disabled="processing"
                    class="inline-flex items-center justify-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground shadow hover:bg-primary/90 disabled:opacity-50"
                >
                    <Save class="size-4" />
                    {{
                        processing
