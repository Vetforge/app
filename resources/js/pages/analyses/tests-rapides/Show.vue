...
const elisaEntries = computed(() =>
    Object.entries(payload.value.elisa ?? {}).filter(
        ([, v]) => v !== '' && v != null,
    ),
);
const biochemEntries = computed(() =>
    Object.entries(payload.value.biochem_rapide ?? {}).filter(
        ([, v]) => v !== '' && v != null,
    ),
);
const pcrEntries = computed(() =>
    Object.entries(payload.value.pcr ?? {}).filter(
        ([, v]) => v !== '' && v != null,
    ),
);
const bandeletteEntries = computed(() =>
    Object.entries(payload.value.bandelette ?? {}).filter(
        ([, v]) => v !== '' && v != null,
    ),
);
const frottisEntries = computed(() =>
    Object.entries(payload.value.frottis ?? {}).filter(
        ([, v]) => v !== '' && v != null,
    ),
);
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        >
            ...
            >
                ...
                <div class="flex flex-wrap gap-2">
                    <div
                        v-for="[key, value] in elisaEntries"
                        :key="key"
                        class="flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5"
                    >
                        <span class="text-sm">{{
                            configuredLabel('elisa_tests', key)
                        }}</span>
                        <span
                            :class="[
            ...
            >
                ...
                <div class="flex flex-wrap gap-2">
                    <div
                        v-for="[key, value] in pcrEntries"
                        :key="key"
                        class="flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5"
                    >
                        <span class="text-sm">{{
                            configuredLabel('pcr_tests', key)
                        }}</span>
                        <span
                            :class="[
                                'rounded px-1.5 py-0.5 text-xs font-medium',
            ...
            >
                ...
                <div class="flex flex-wrap gap-2">
                    <div
                        v-for="[key, value] in biochemEntries"
                        :key="key"
                        class="flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5"
                    >
                        <span class="text-sm">{{
                            configuredLabel('biochem_rapide', key)
                        }}</span>
            ...
            <!-- Bandelette -->
            <section
                v-if="bandeletteEntries.length > 0"
                class="rounded-xl border border-border bg-card p-3 sm:p-5"
            >
                <p
                    class="mb-3 text-xs font-semibold text-muted-foreground uppercase"
            ...
            >
                ...
                >
                    {{
                        configuredLabel('optional_sections', 'frottis_sanguin')
                    }}
                </p>
                <div class="flex flex-wrap gap-2">
                    <div
                        v-for="[key, value] in frottisEntries"
                        :key="key"
                        class="flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5"
                    >
                        <span class="text-sm">{{
