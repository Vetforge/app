...
const paramsByGroup = computed<Record<string, ParamDef[]>>(() => {
    ...
    const enabled = all.filter(p => p.enabled !== false && matchesSpecies(p.species, form.payload.species));
...
watch(() => form.payload.species, () => {
    const validParamKeys = new Set(
        Object.values(paramsByGroup.value)
            .flat()
            .map(param => param.key),
    );
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <form class="analysis-form mx-auto flex max-w-7xl flex-col gap-3 p-3 sm:p-4" @submit.prevent="submit">
            ...
            <!-- NFS par groupe -->
            <section v-for="(params, group) in paramsByGroup" :key="group" class="analysis-section">
                <div class="analysis-section-heading">
                    <div><h2>{{ GROUP_LABELS[group] ?? group }}</h2></div>
                </div>
