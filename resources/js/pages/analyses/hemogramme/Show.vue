...
interface ParamDef { key: string; label: string; group: string; species?: string[]; enabled: boolean }
...
const paramsByGroup = computed<Record<string, ParamDef[]>>(() => {
    ...
    const enabled = all.filter(p => p.enabled !== false && matchesSpecies(p.species, payload.value.species) && payload.value.params[p.key] !== undefined && payload.value.params[p.key] !== '');
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="analysis-show mx-auto flex w-full min-w-0 max-w-5xl flex-col gap-4 px-3 py-4 sm:gap-6 sm:p-6">
            ...
            <template v-for="(params, group) in paramsByGroup" :key="group">
                <section class="min-w-0 rounded-xl border border-border bg-card p-3 sm:p-5">
                    <p class="mb-3 text-xs font-semibold uppercase text-muted-foreground">{{ GROUP_LABELS[group] ?? group }}</p>
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            <tr v-for="param in params" :key="param.key" class="border-b border-border/50">
