...
const paramsByGroup = computed<Record<string, ParamDef[]>>(() => {
    const all: ParamDef[] = Array.isArray(settings.value.params)
        ? settings.value.params
        : [];
    const enabled = all.filter(
        (p) =>
            p.enabled !== false &&
            matchesSpecies(p.species, payload.value.species) &&
            payload.value.params[p.key] !== undefined &&
            payload.value.params[p.key] !== '',
    );
        ...
        >
            ...
            >
                ...
                <div>
                    ...
                    <p class="text-sm">
                        Prel. : {{ formatDate(analysis.sampled_at) }}
                    </p>
                    <p class="text-sm">
                        Anal. : {{ formatDate(analysis.analyzed_at) }}
                    </p>
                </div>
                <div>
                    <p class="text-xs text-muted-foreground uppercase">
            ...
            <template v-for="(params, group) in paramsByGroup" :key="group">
                <section
                    class="min-w-0 rounded-xl border border-border bg-card p-3 sm:p-5"
                >
                    <p
                    ...
                    <table class="w-full text-sm">
                        <thead class="hidden sm:table-header-group">
                            ...
                            >
                                <th class="py-1 pr-4 text-left font-medium">
                                    Parametre
                                </th>
                                <th class="py-1 pr-4 text-right font-medium">
                                    Valeur
                                </th>
                                <th class="py-1 pr-4 text-left font-medium">
                                    Normes
                        ...
                        <tbody>
                            ...
                            >
                                <td
                                    class="py-1.5 pr-4 text-xs font-medium text-muted-foreground"
                                >
                                    {{ param.label }}
                                    <div
                                        class="mt-0.5 flex flex-wrap items-center gap-x-2 font-normal sm:hidden"
                                        :class="
                                            statusClass(
                                                valueStatus(
                                                    param.key,
                                ...
                                <td
                                    :class="[
                                        'hidden py-1.5 pr-4 text-right font-semibold tabular-nums sm:table-cell',
                                        statusClass(
                                            valueStatus(
                                                param.key,
                                                payload.params[param.key],
                                ...
                                <td
                                    class="hidden py-1.5 pr-4 text-xs text-muted-foreground sm:table-cell"
                                >
                                ...
                                <td
                                    class="hidden py-1.5 text-xs text-muted-foreground sm:table-cell"
                                >
            ...
            <section
                v-if="payload.commemoratifs || payload.commentaires"
