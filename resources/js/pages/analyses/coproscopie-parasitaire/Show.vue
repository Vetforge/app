...
const visibleParasites = computed<Array<{ key: string; label: string; species?: string[]; requires_option?: string }>>(() =>
    ((settings.value.parasites as Array<any>) ?? []).filter((parasite: any) => {
        ...
        return requiredOption === '' || (payload.value.options as Record<string, boolean> | undefined)?.[requiredOption] === true;
    }),
);
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="analysis-show mx-auto flex w-full min-w-0 max-w-5xl flex-col gap-4 px-3 py-4 sm:gap-6 sm:p-6">
            ...
            <section class="min-w-0 space-y-4 rounded-xl border border-border bg-card p-3 sm:p-5">
                    ...
                    <table class="w-full text-sm">
                        <thead class="hidden sm:table-header-group">
                            <tr class="border-b border-border bg-muted/40">
                                ...
                                <th v-for="parasite in visibleParasites" :key="parasite.key" class="px-3 py-2 text-left font-medium">{{ parasite.label }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(sample, i) in (payload.samples as any[])" :key="i" class="border-b border-border/50 last:border-b-0">
                                <td class="px-3 py-2">
                                    ...
                                    <div class="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-xs sm:hidden">
                                        <span v-for="parasite in visibleParasites" :key="parasite.key" :class="pathogenResultClass(sample.results?.[parasite.key])">
                                            {{ parasite.label }} : {{ scaleLabel(sample.results?.[parasite.key] ?? '0') }}
                                        </span>
                                    </div>
                                </td>
                                <td v-for="parasite in visibleParasites" :key="parasite.key" class="hidden px-3 py-2 sm:table-cell" :class="pathogenResultClass(sample.results?.[parasite.key])">
                ...
                <div v-if="results.positive_parasites && Object.keys(results.positive_parasites as object).length > 0" class="space-y-2">
                    ...
                    <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        >
                            ...
                            <p class="text-xs text-amber-700">{{ parasite.positive_samples }} ech. positif(s) · Score max : {{ parasite.max_score }}</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="flex flex-col gap-4 rounded-xl border border-border bg-card p-3 sm:p-5">
                <div class="grid gap-4 lg:grid-cols-2">
                    <div>
                        <p class="mb-1 text-xs font-semibold uppercase text-muted-foreground">Conseils preventifs</p>
                        <div class="text-sm" v-html="formatResult(payload.advice_preventive)"></div>
                    </div>
                    <div>
                        <p class="mb-1 text-xs font-semibold uppercase text-muted-foreground">Conseils curatifs</p>
                        <div class="text-sm" v-html="formatResult(payload.advice_curative)"></div>
                    </div>
                </div>
            </section>
        </div>
