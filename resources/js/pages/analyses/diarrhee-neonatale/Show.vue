...
const visiblePathogens = computed<Array<{ key: string; label: string; tests?: string[]; requires_option?: string }>>(() =>
    ((settings.value.pathogens as Array<any>) ?? []).filter((pathogen: any) => pathogen.enabled !== false && isVisiblePathogen(pathogen)),
);
...
function isVisiblePathogen(pathogen: Record<string, any>): boolean {
    if ((pathogen.requires_option === 'coccidiosis_test' || pathogen.key === 'coccidies') && payload.value.coccidiosis_test !== true) {
        return false;
    }
            ...
            <section class="min-w-0 space-y-4 rounded-xl border border-border bg-card p-3 sm:p-5">
                ...
                <div class="rounded-lg border border-border">
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            <tr v-for="pathogen in visiblePathogens" :key="pathogen.key" class="border-b border-border/50 last:border-b-0">
                                <td class="px-3 py-2">
