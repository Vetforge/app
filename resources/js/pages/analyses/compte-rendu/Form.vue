...
function addPage(): void {
    if (form.payload.pages.length < 10) {
        form.payload.pages.push('');
        form.payload.nb_pages = form.payload.pages.length;
    }
}

function removePage(index: number): void {
    if (form.payload.pages.length > 1) {
        form.payload.pages.splice(index, 1);
        form.payload.nb_pages = form.payload.pages.length;
    }
}
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <form class="analysis-form mx-auto flex max-w-7xl flex-col gap-3 p-3 sm:p-4" @submit.prevent="submit">
            ...
            <section class="analysis-section">
                <div class="analysis-section-heading">
                    <div>
                        ...
                        <p>Eleveur, animal et date du compte-rendu.</p>
                    </div>
                </div>
            ...
            <section
                v-for="(page, index) in form.payload.pages"
                :key="index"
                class="analysis-section"
            >
                <div class="analysis-section-heading">
                    <div>
                        <h2>Page {{ index + 1 }} / {{ form.payload.pages.length }}</h2>
                    </div>
                    <button
                        v-if="form.payload.pages.length > 1"
                        type="button"
                        class="inline-flex items-center gap-1 rounded-md border border-destructive/40 px-2 py-1 text-xs font-medium text-destructive hover:bg-destructive/10"
                ...
                <div class="grid gap-1">
                    <textarea
                        v-model="form.payload.pages[index]"
                        rows="20"
                        class="rounded border border-border bg-background px-2 py-1 text-sm"
                        :placeholder="`Contenu de la page ${index + 1}...`"
                    ></textarea>
                </div>
            </section>

            <div v-if="form.payload.pages.length < 10" class="flex justify-center">
                <button
                    type="button"
                    class="inline-flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-xs font-medium hover:bg-accent"
                    @click="addPage"
                >
                    <Plus class="size-3.5" />
                    Ajouter une page
